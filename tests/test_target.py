...
@pytest.mark.parametrize('target_feature', ['a', 'c', 'e'])
def test_only_target(raw_data, target_feature):
    (X_train_raw_df,
     X_test_raw_df,
     gene_features) = raw_data

    X_train_df, gene_features_train = tu.only_target_from_data(X_train_raw_df,
                                                               target_feature,
                                                               gene_features)
    X_test_df, gene_features_test = tu.only_target_from_data(X_test_raw_df,
                                                             target_feature,
                                                             gene_features)
    ...
    # target feature should be present in columns
    assert (target_feature in X_train_df.columns)
    assert (target_feature in X_test_df.columns)

    # other features should not be present
    assert (X_train_df.columns[gene_features_train].equals(
            pd.Index([target_feature])))
    assert (X_test_df.columns[gene_features_test].equals(
            pd.Index([target_feature])))
